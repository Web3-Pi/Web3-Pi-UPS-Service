...
impl Default for HostMetricsConfig {
    fn default() -> Self {
        // 30 s, sized against the M.2 modem's 1NCE data pool: 500 MB per SIM
        // for its whole life (10-year validity), NOT a monthly plan — measured
        // per-frame packet size. 0 disables. Host metrics only
